...
class FollowPageState extends State<FollowPage> {
  ...
  Widget build(BuildContext context) {
    log("-------------------FollowPageState-------build");
    // return Container( child: Text("yes is me",style: TextStyle(color: Colors.white, fontSize: 40),),);
    return lstPost.length == 0 ?
      Row(
  ...
  Future<void> LoadFollowPost({int offset = 0, int count = 20,}) async {
    var req = $pbCommunity.PostsReq();
    req.type = $pbCommunity.PostsReq_Type.Followee;
    ...
    RpcCallImOuterApi(
      ...
      // log("load community post list ok, ${rsp.posts}");
  ...
  Widget BuildPosts(BuildContext context, int index) {
    ...
    return InkWell(
      ...
      child: PostCard(post,
          RelationFollow.isFollowSync(post.userId)!,OptBuildType.PostSummary,),
    );
  }
}
