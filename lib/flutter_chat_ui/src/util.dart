...
String getVerboseDateTimeRepresentation(
  ...
  return secToTimeMessage(dateTime.millisecondsSinceEpoch);
}
...
List<Object> calculateChatMessages(
  ...
  for (var i = messages.length - 1; i >= 0; i--) {
    ...
    if (messageHasCreatedAt && nextMessageHasCreatedAt) {
      ...
      if (nextMessage.type == types.MessageType.system) {
    ...
    if (message.status == types.Status.sending || message.status == types.Status.error) {
      showStatus = message.showStatus ?? true;
    } else {
      if (GlobalController.to.systemConfig.value?.showMessageReadStatus == false) {
        showStatus = false;
      } else {
        showStatus = message.showStatus ?? true;
      }
    ...
    if (message is types.ImageMessage) {
      if (kIsWeb) {
        if (message.uri.startsWith('http') || message.uri.startsWith('blob')) {
          gallery.add(PreviewImage(id: message.id, uri: message.uri, author: message.author));
        }
      } else {
        gallery.add(PreviewImage(id: message.id, uri: message.uri, author: message.author));
      }
    ...
    if (message is types.VideoMessage) {
      ...
      if (kIsWeb) {
        if (message.uri.startsWith('http') || message.uri.startsWith('blob')) {
          videos.add(PreviewVideo(id: message.id, uri: message.uri, thumbnail: thumbnailUrl, author: message.author));
        }
      } else {
        videos.add(PreviewVideo(id: message.id, uri: message.uri, thumbnail: thumbnailUrl, author: message.author));
      }
  ...
  return [chatMessages, gallery, videos];
}
